...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/04 15:03:16 by jvaquer           #+#    #+#             */
/*   Updated: 2019/11/19 16:45:12 by jvaquer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_count			ft_check_null_str(t_count cmp, char *s)
{
	if (s[cmp.i - 1] == '0' && s[cmp.i - 2] == '%')
		return (cmp);
	if (cmp.i >= 2 && s[cmp.i] == 'd' && s[cmp.i - 1] == '0'
		&& s[cmp.i - 2] == '0' && cmp.check == 1)
		return (cmp);
	if ((s[cmp.i] == 'x' || s[cmp.i] == 'X' || s[cmp.i] == 'u')
		&& (cmp.zero == -4294967295 || cmp.zero == 0) &&
