...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/01 15:05:11 by jvaquer           #+#    #+#             */
/*   Updated: 2019/11/19 16:41:13 by jvaquer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_count		ft_flag_z(va_list aux, t_count cmp, const char *s)
{
	...
	cmp.check = 1;
	cmp.i++;
	cmp = (s[k] == '.') ? ft_flag_p(aux, cmp, s) : cmp;
	if (s[k] == '*')
	{
...
t_count		ft_flag_p(va_list aux, t_count cmp, const char *s)
{
	...
	cmp.i++;
	cmp = (s[k] == '.' && s[k - 1] == '0') ? ft_flag_p(aux, cmp, s) : cmp;
	if (s[k] == '*')
	{
		if (ft_convertible(s[k + 1]) == 1)
		{
...
t_count		ft_flag_m(va_list aux, t_count cmp, char *s)
{
	...
	{
		cmp.i++;
		cmp = ft_flag_ptr(aux, (const char *)s, cmp);
	...
	return ((cmp = s[k] == '.' ? ft_flag_p(aux, cmp, s) : cmp));
}
